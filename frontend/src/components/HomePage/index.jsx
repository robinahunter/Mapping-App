...
const HomePage = () => {
  ...
  useEffect(() => {
    const initializeArcGIS = async () => {
      ...
      // Initialize ArcGIS components after API key is set
      if (!loading) {
        // Create a VectorTileLayer from a style URL
        const mapBaseLayer = new VectorTileLayer({
          url:
            'https://arcgis.com/sharing/rest/content/items/b5676525747f499687f12746441101ef/resources/styles/root.json',
        });
        ...
        const featureLayer = new FeatureLayer({
          url:
            'https://services8.arcgis.com/T51xV83kzxEuohfL/arcgis/rest/services/maui_fresh_water_fountain_locations/FeatureServer',
        });
        ...
        const renderer = {
          ...
          symbol: {
            type: 'simple-marker',
            url:
              'https://drive.google.com/uc?id=1RE4mTAvWbMKh9XQ9US5Nx_7TXgDjZUkt',
            color: '#06b6d4',
            size: 8,
            outline: {
              color: [0, 0, 0, 0], // Specify transparent outline
    ...
    <>
      <div style={{ position: 'relative', minHeight: '100vh' }}>
        {loading && (
          ...
          >
            <TailSpin color="#06b6d4" height={50} width={50} />
            <p>Loading Map...</p>
          </div>
        )}
